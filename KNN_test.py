...
def run():
        ...
        list = os.listdir("testDigits") #测试集文件夹
        x_test = np.zeros((1,1024))     #初始化测试数组
        y_test=[]                       #初始化测试标签
        j=0                             #用于判断是不是第一次进入循环
        for i in list:
                data=img2vector(root+"\\testDigits\\"+i)
                if(j==0):
                        x_test=data[:]  #第一次需要赋值
                        j=j+1
                else:
                        x_test = np.vstack((x_test,data[:]))#之后每一次都在最下面一行底下添加一行
                y_test.append(int(i.split("_")[0])) #将训练标签导入
        result = knn.predict(x_test)    #进行预测
        print(np.sum(result == y_test)/len(y_test))     #输出正确率
